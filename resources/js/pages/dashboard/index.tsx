...
export default function Dashboard({
    ...
    return (
        <DashboardLayout title="Tableau de bord" subtitle="Bienvenue sur votre espace STAT ENQUETE">
...
function KpiCard({ label, value, sub, subUp = false, icon, accent = 'blue', progress }: {
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-100 p-6">
            ...
            <p className="text-2xl font-extrabold text-[#0f172a] tracking-tight">{value}</p>
            {progress !== undefined && (
...
function EnqueteRow({ initials, titre, reponses, statut, creee_le }: Omit<EnqueteRecente, 'id'>) {
    ...
    return (
        <div className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50 transition-colors group">
            ...
            <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full border ${statusStyle}`}>
...
function QuickAction({ href, icon, label, color }: {
    ...
    return (
        <Link href={href} className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-slate-50 transition-colors group">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${color}`}>{icon}</div>
            <span className="text-sm font-medium text-slate-600 group-hover:text-[#0f172a] transition-colors">{label}</span>
            <svg className="w-4 h-4 text-slate-300 ml-auto group-hover:text-[#2563eb] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
