...
interface Props {
    ...
    stats:    { total: number; ce_mois: number };
...
export default function Contacts({ contacts, stats, filters = {}, success }: Props) {
    ...
    const toggleAll = () =>
        setSelected(allSelected ? [] : contacts.data.map(c => c.id));
    ...
    return (
        <DashboardLayout title="Contacts" subtitle="Votre carnet d'adresses">
            ...
            {/* ── Barre sélection ─────────────────────────────────────────── */}
            {selected.length > 0 && (
                <div className="flex items-center justify-between mb-4 px-5 py-3 bg-blue-50 border border-blue-200 rounded-2xl">
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-slate-100">
                                ...
                                {['Contact', 'Email', 'Notes', 'Ajouté le', 'Actions'].map(h => (
                                    <th key={h} className={`px-4 py-4 text-xs font-bold uppercase tracking-wider text-slate-400 ${h === 'Actions' ? 'text-right' : 'text-left'}`}>
                                        {h}
                                    </th>
                                ))}
                            </tr>
                                ...
                                <tr key={c.id} className={`hover:bg-slate-50 transition-colors group ${selected.includes(c.id) ? 'bg-blue-50/40' : ''}`}>
                                    ...
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-sm font-semibold text-[#0f172a]">{c.nom}</span>
                                        </div>
                                    </td>
                                    {/* Email */}
                                    <td className="px-4 py-4">
                                    ...
                                    {/* Date */}
                                    <td className="px-4 py-4 text-sm text-slate-400">{c.created_at}</td>
                                    {/* Actions */}
                                    <td className="px-4 py-4">
                                        <div className="flex items-center justify-end gap-1">
                                            ...
                                            <button onClick={() => navigator.clipboard.writeText(c.email)}
                                                className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-green-600 transition-colors" title="Copier l'email">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                            </button>
                                            <button onClick={() => setDelContact(c)}
                ...
                {contacts.last_page > 1 && (
                    <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4">
                        <p className="text-sm text-slate-400">
                            <span className="font-semibold text-[#0f172a]">{contacts.total.toLocaleString()}</span> contact{contacts.total > 1 ? 's' : ''}
                        </p>
            ...
            {/* ── MODAL CRÉER ─────────────────────────────────────────────── */}
            {showCreate && (
                <ContactModal onClose={() => setShowCreate(false)} />
            )}
            ...
            {/* ── MODAL SUPPRIMER ─────────────────────────────────────────── */}
            {delContact && (
                <DeleteModal contact={delContact} onClose={() => setDelContact(null)} />
            )}
...
function ContactModal({ contact, onClose }: { contact?: ContactItem; onClose: () => void }) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        if (contact) {
            form.put(`/contacts/${contact.id}`, { onSuccess: () => { form.reset(); onClose(); } });
        } else {
            form.post('/contacts', { onSuccess: () => { form.reset(); onClose(); } });
        }
    };
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                            {contact ? contact.email : 'Ajouter un contact à votre carnet'}
                        </p>
                    </div>
                ...
                <form onSubmit={submit} className="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" value={form.data.nom}
                            ...
                            placeholder="Alphonse loic" autoFocus />
                    ...
                    <div>
                        ...
                        <textarea rows={3} value={form.data.notes}
            ...
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">Fichier CSV avec colonnes : nom, email, notes</p>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-200 transition-colors">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                ...
                <form onSubmit={submit} className="p-6 space-y-5">
                    {/* Zone de dépôt */}
                    <label className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center cursor-pointer transition-all ${
                        form.data.csv ? 'border-[#2563eb] bg-blue-50' : 'border-slate-200 hover:border-blue-300 hover:bg-slate-50'
                    }`}>
                        <svg className={`w-10 h-10 mb-3 ${form.data.csv ? 'text-[#2563eb]' : 'text-slate-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/>
                        </svg>
                        {form.data.csv ? (
                            <p className="text-sm font-semibold text-[#2563eb]">{form.data.csv.name}</p>
                        ) : (
                            <>
                                <p className="text-sm font-semibold text-slate-500">Glisser un fichier CSV</p>
                                <p className="text-xs text-slate-400 mt-1">ou <span className="text-[#2563eb] font-semibold">cliquer pour parcourir</span></p>
                            </>
                        )}
                    ...
                    <div className="bg-slate-50 rounded-xl border border-slate-100 p-4">
                        <p className="text-xs font-semibold text-slate-500 mb-2">Format attendu (ligne 1 = en-tête ignorée) :</p>
                        <code className="text-xs text-slate-600 font-mono block">nom,email,notes</code>
                        <code className="text-xs text-slate-400 font-mono block mt-1">Alphonse loic,alphonse@mail.com,Client VIP</code>
                    </div>
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        <button type="submit" disabled={!form.data.csv || form.processing}
                            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#2563eb] text-white text-sm font-semibold hover:bg-[#1d4ed8] disabled:opacity-50 transition-colors">
                            {form.processing ? <Spin /> : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6">
                ...
                <p className="text-sm text-slate-600 mb-6">
                    Supprimer <span className="font-semibold text-[#0f172a]">{contact.nom}</span> de votre carnet d'adresses ?
                </p>
                <div className="flex justify-end gap-3">
                    <button onClick={onClose} className="px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">
                        Annuler
                    </button>
