...
interface Props {
    ...
    forms: FormOption[];
    stats: Stats;
    filters?: { search?: string; form_id?: string };
}
...
export default function ResponsesIndex({ responses, forms, stats, filters = {} }: Props) {
    ...
    const safeStats = {
        total:         stats?.total          ?? 0,
        aujourd_hui:   stats?.aujourd_hui    ?? 0,
        cette_semaine: stats?.cette_semaine  ?? 0,
        ce_mois:       stats?.ce_mois        ?? 0,
    };

    const applyFilters = (overrides: object = {}) => {
        router.get('/reponses', { search, form_id: formId, ...overrides }, { preserveState: true });
    };
            ...
            <div className="grid gap-4 sm:grid-cols-4 mb-6">
                {[
                    ...
                    <div key={s.label} className="bg-white rounded-2xl border border-slate-100 p-5">
                        ...
                        <p className={`mt-1 text-2xl font-extrabold tracking-tight ${s.color}`}>{s.value.toLocaleString()}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
                <div className="relative flex-1 max-w-md">
                    <input type="text" value={search}
                        ...
                        placeholder="Rechercher par IP, enquête…"
                ...
                <select value={formId}
                    onChange={e => { setFormId(e.target.value); applyFilters({ form_id: e.target.value }); }}
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-slate-100">
                                {['Enquête', 'Adresse IP', 'Réponses', 'Soumis le', 'Actions'].map(h => (
                                ...
                                <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-semibold text-[#0f172a] group-hover:text-[#2563eb] transition-colors">{r.form_title}</p>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#2563eb] bg-blue-50 px-2.5 py-1 rounded-full">
                                            {r.answers_count} champ{r.answers_count > 1 ? 's' : ''}
                                        </span>
                                    </td>
                                    {/* Date */}
                                    <td className="px-6 py-4 text-sm text-slate-500">{r.submitted_at}</td>
                                    {/* Actions */}
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-end gap-1">
                                            <Link href={`/reponses/${r.form_id}`}
                                                className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-[#2563eb] transition-colors" title="Voir les détails">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                            </Link>
                                            <button onClick={() => handleDelete(r.id)} disabled={deleting === r.id}
                                                className="p-2 rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors disabled:opacity-50" title="Supprimer">
                                                {deleting === r.id ? (
                ...
                {responses.last_page > 1 && (
                    <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4">
                        <p className="text-sm text-slate-400">
                            <span className="font-semibold text-[#0f172a]">{responses.total.toLocaleString()}</span> réponse{responses.total > 1 ? 's' : ''}
                        </p>
