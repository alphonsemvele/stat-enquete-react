                        ...
                        <tbody className="divide-y divide-slate-50">
                            {forms.data.length === 0 ? (
                                ...
                                <tr key={form.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4"><Link href={`/reponses/${form.id}`} className="text-sm font-semibold text-[#2563eb] hover:underline">{form.responses_count}</Link></td>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-end gap-1">
                                            ...
                                            {/* Voir les réponses */}
                                            <Link href={`/reponses/${form.id}`}
                                                className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-indigo-600 transition-colors" title="Voir les réponses">
                                                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4"/></svg>
                                            </Link>
                                            {/* Invitations */}
                                            <Link href={`/invitations?form_id=${form.id}`}
                                                className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-violet-600 transition-colors" title="Envoyer des invitations">
