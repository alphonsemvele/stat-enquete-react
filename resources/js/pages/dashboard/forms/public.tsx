...
interface QProps {
    ...
    rows?: string; min?: string; max?: string; options?: string;
    accept?: string; title?: string; subtitle?: string; size?: string;
    [key: string]: unknown;
}
...
export default function PublicForm({ form, questions }: Props) {
    ...
    const validate = (): boolean => {
        ...
        questions.forEach(q => {
            if (q.properties.required && q.type !== 'block_title') {
                ...
                if (!val.trim()) {
                    newErrors[q.id] = (q.properties.label ?? 'Ce champ') + ' est obligatoire.';
                }
            }
        });
        setErrors(newErrors);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post(`/f/${form.reference}`, { answers }, {
            onError: (errs) => {
                setSubmitting(false);
                // Mapper les erreurs Laravel vers les IDs de questions
                const mapped: Record<number, string> = {};
                Object.entries(errs).forEach(([key, msg]) => {
                    const match = key.match(/answers\.(\d+)/);
                    if (match) mapped[parseInt(match[1])] = msg as string;
                });
                setErrors(mapped);
            },
        });
    ...
    const visibleQuestions = questions.filter(q => q.type !== 'block_title' || true);
        ...
        <>
            ...
            <div className="min-h-screen bg-slate-50 py-10 px-4">
                <div className="max-w-2xl mx-auto">
                    ...
                    {/* Header */}
                    <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden mb-5 shadow-sm">
                        <div className="h-2" style={{ backgroundColor: form.color }}></div>
                        <div className="px-8 py-7">
                            <h1 className="text-2xl font-bold text-[#0f172a] mb-2">{form.title}</h1>
                            {form.description && (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="pt-2">
                            <button type="submit" disabled={submitting}
                                className="w-full py-3.5 rounded-xl font-semibold text-white text-sm transition-all disabled:opacity-60 hover:opacity-90 shadow-sm"
...
function QuestionField({ question: q, index, value, error, onChange, color }: {
    question: Question;
    index: number;
    value: string;
    error?: string;
    ...
    const base = `w-full px-4 py-3 rounded-xl border text-sm text-[#0f172a] transition-all outline-none focus:ring-2 ${
        error ? 'border-red-300 focus:border-red-400 focus:ring-red-50' : 'border-slate-200 focus:ring-blue-50'
    }`;

    // Bloc titre — pas un champ, juste un séparateur visuel
    if (q.type === 'block_title') {
        const sz = q.properties.size === 'h1' ? 'text-2xl' : q.properties.size === 'h3' ? 'text-base' : 'text-xl';
        return (
            <div className="bg-white rounded-2xl border border-slate-100 px-8 py-6 shadow-sm">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 px-8 py-6 shadow-sm">
            ...
            {q.type === 'text_input' && (
                <input type="text" value={value} onChange={e => onChange(e.target.value)}
                    placeholder={q.properties.placeholder} maxLength={q.properties.maxLength ? parseInt(q.properties.maxLength as string) : undefined}
                    className={base} style={{ '--tw-ring-color': color + '40' } as any} />
            )}

            {q.type === 'textarea' && (
                <textarea value={value} onChange={e => onChange(e.target.value)}
                    placeholder={q.properties.placeholder} rows={parseInt(q.properties.rows ?? '4')}
                    className={`${base} resize-none`} />
            )}

            {q.type === 'number_input' && (
                <input type="number" value={value} onChange={e => onChange(e.target.value)}
                    placeholder={q.properties.placeholder}
                    min={q.properties.min || undefined} max={q.properties.max || undefined}
                    className={base} />
            )}

            {q.type === 'email' && (
                <input type="email" value={value} onChange={e => onChange(e.target.value)}
                    placeholder={q.properties.placeholder} className={base} />
            )}
            ...
            {q.type === 'radio' && (
                <div className="space-y-2.5">
                    {(q.properties.options ?? '').split(',').filter(o => o.trim()).map((o, i) => (
                        ...
                        }`} style={value === o.trim() ? { borderColor: color, backgroundColor: color + '10' } : {}}>
                            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${value === o.trim() ? 'border-current' : 'border-slate-300'}`}
                                style={value === o.trim() ? { borderColor: color } : {}}>
                                {value === o.trim() && <div className="w-2 h-2 rounded-full" style={{ backgroundColor: color }}></div>}
                            </div>
                            <input type="radio" name={`q-${q.id}`} value={o.trim()} checked={value === o.trim()}
            ...
            {q.type === 'checkbox' && (
                <label className={`flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer transition-all ${
                    value === 'true' ? 'border-2 bg-blue-50' : 'border-slate-200 hover:border-slate-300'
                }`} style={value === 'true' ? { borderColor: color, backgroundColor: color + '10' } : {}}>
                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 ${value === 'true' ? 'border-current' : 'border-slate-300'}`}
                        style={value === 'true' ? { borderColor: color, backgroundColor: color } : {}}>
            ...
            {q.type === 'file_upload' && (
                <label className="border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center cursor-pointer hover:border-blue-300 hover:bg-blue-50 transition-all">
                    ...
                    <input type="file" className="sr-only" accept={q.properties.accept || undefined}
