...
interface Props { membres: Membre[]; stats: Stats; filters?: { search?: string; role?: string }; me: number; success?: string; }
...
const ROLE_LABELS: Record<string, string> = { admin: 'Administrateur', user: 'Utilisateur' };
const AVATAR_COLORS = ['#2563eb','#7c3aed','#059669','#dc2626','#d97706','#0891b2','#db2777','#65a30d','#ea580c','#4f46e5'];
...
export default function Equipe({ membres, stats, filters = {}, me, success }: Props) {
    ...
    const safeStats = { total: stats?.total ?? 0, admins: stats?.admins ?? 0, users: stats?.users ?? 0 };
    ...
    return (
        <DashboardLayout title="Équipe" subtitle="Gérez les membres et leurs accès">
            ...
            {membres.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 flex flex-col items-center justify-center py-20 text-center">
                    <div className="w-14 h-14 rounded-2xl bg-slate-100 flex items-center justify-center mb-4">
                        <svg className="w-7 h-7 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
                    </div>
                        ...
                        <div key={m.id} className="bg-white rounded-2xl border border-slate-100 p-5 hover:shadow-md hover:shadow-slate-100 transition-all group">
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <div className="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex-shrink-0">
                                    <button onClick={() => setEditMembre(m)} className="p-1.5 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-[#2563eb] transition-colors">
                                    ...
                                    {!m.is_me && (
                                        <button onClick={() => setDelMembre(m)} className="p-1.5 rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors">
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-2">
                                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${ROLE_COLORS[m.role]}`}>{ROLE_LABELS[m.role]}</span>
                                <span className="text-xs text-slate-400">Depuis le {m.created_at}</span>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {showCreate  && <MembreModal title="Ajouter un membre" subtitle="Créer un nouveau compte utilisateur" onClose={() => setShowCreate(false)} />}
            {editMembre  && <MembreModal title="Modifier le membre" subtitle={editMembre.name} membre={editMembre} onClose={() => setEditMembre(null)} />}
            {delMembre   && <DeleteModal membre={delMembre} onClose={() => setDelMembre(null)} />}
        </DashboardLayout>
    );
}

function MembreModal({ title, subtitle, membre, onClose }: { title: string; subtitle: string; membre?: Membre; onClose: () => void }) {
    const form = useForm({ name: membre?.name ?? '', email: membre?.email ?? '', role: membre?.role ?? 'user', password: '', password_confirmation: '' });
    const [showPass, setShowPass] = useState(false);

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        if (membre) { form.put(`/equipe/${membre.id}`, { onSuccess: () => { form.reset(); onClose(); } }); }
        else        { form.post('/equipe',              { onSuccess: () => { form.reset(); onClose(); } }); }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 sticky top-0 bg-white z-10">
                    <div><h2 className="text-base font-bold text-[#0f172a]">{title}</h2><p className="text-xs text-slate-400 mt-0.5">{subtitle}</p></div>
                ...
                <form onSubmit={submit} className="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" value={form.data.name} onChange={e => form.setData('name', e.target.value)} className={inputCls(form.errors.name)} placeholder="Alphonse Loic" autoFocus />
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {(['user', 'admin'] as const).map(r => (
                                <button key={r} type="button" onClick={() => form.setData('role', r)}
                                    className={`flex items-center gap-3 p-3.5 rounded-xl border-2 text-left transition-all ${form.data.role === r ? 'border-[#2563eb] bg-blue-50' : 'border-slate-200 hover:border-slate-300'}`}>
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${form.data.role === r ? 'bg-[#2563eb]' : 'bg-slate-100'}`}>
                                        <svg className={`w-4 h-4 ${form.data.role === r ? 'text-white' : 'text-slate-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            {r === 'admin' ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                                                           : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>}
                                        </svg>
                                    </div>
                                    <div>
                                        <p className={`text-xs font-bold ${form.data.role === r ? 'text-[#2563eb]' : 'text-[#0f172a]'}`}>{r === 'admin' ? 'Admin' : 'Utilisateur'}</p>
                                        <p className="text-[10px] text-slate-400">{r === 'admin' ? 'Accès complet' : 'Accès standard'}</p>
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    <div>
                        <label className={labelCls}>{membre ? 'Nouveau mot de passe (laisser vide pour ne pas changer)' : 'Mot de passe *'}</label>
                        <div className="relative">
                            <input type={showPass ? 'text' : 'password'} value={form.data.password} onChange={e => form.setData('password', e.target.value)} className={inputCls(form.errors.password) + ' pr-12'} placeholder="Minimum 8 caractères" />
...
function DeleteModal({ membre, onClose }: { membre: Membre; onClose: () => void }) {
    ...
    const handleDelete = () => { setLoading(true); router.delete(`/equipe/${membre.id}`, { onSuccess: () => onClose(), onFinish: () => setLoading(false) }); };
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6">
                <div className="flex items-center gap-4 mb-5">
                    ...
                    <div><h2 className="font-bold text-[#0f172a]">Supprimer le membre</h2><p className="text-xs text-slate-400 mt-0.5">Cette action est irréversible</p></div>
                </div>
                <p className="text-sm text-slate-600 mb-6">Êtes-vous sûr de vouloir supprimer <span className="font-semibold text-[#0f172a]">{membre.name}</span> ?</p>
